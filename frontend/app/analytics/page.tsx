...
export default function AnalyticsPage() {
  ...
  const load = async () => {
    ...
    try {
      const [rh, tr, sc] = await Promise.all([
        fetchRiskHistory(hours), fetchTraffic(hours * 2), fetchTrafficWeather(hours),
      ]);
      setRiskHistory(rh.data); setTraffic(tr.traffic); setScatter(sc.data);
  ...
  const tt = { contentStyle: { background: "#141A2F", border: "1px solid #1E293B", borderRadius: 8, fontFamily: "monospace", fontSize: 11 } };
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <div className="flex flex-col flex-1 ml-60 overflow-hidden">
        ...
        <motion.main className="flex-1 overflow-y-auto p-5 space-y-5"
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}>

          {/* Time range */}
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-text-muted font-mono uppercase tracking-widest mr-2">Range:</span>
            {HOURS.map((h) => (
          ...
          <Card>
            ...
            <div className="h-64">
              {loading ? <div className="w-full h-full bg-border/20 rounded animate-pulse" /> :
               !riskHistory.length ? <p className="text-text-secondary text-sm text-center mt-16">Awaiting data...</p> :
          ...
          <Card>
            ...
            <div className="h-64">
              {loading ? <div className="w-full h-full bg-border/20 rounded animate-pulse" /> :
               !traffic.length ? <p className="text-text-secondary text-sm text-center mt-16">Awaiting data...</p> :
          ...
          <Card>
            ...
            <p className="text-[10px] text-text-muted font-mono mb-4">
              Dot size = congestion ratio
            </p>
            <div className="h-64">
              {loading ? <div className="w-full h-full bg-border/20 rounded animate-pulse" /> :
               !scatter.length ? <p className="text-text-secondary text-sm text-center mt-16">Awaiting data...</p> : (
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart margin={{ top: 8, right: 8, left: -24, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#1E293B" />
                    <XAxis dataKey="rainfall_1h" name="Rainfall mm"
                      tick={{ fill: "#475569", fontSize: 10, fontFamily: "monospace" }}
                      axisLine={{ stroke: "#1E293B" }} tickLine={false} />
                    <YAxis dataKey="incident_count" name="Incidents"
                      tick={{ fill: "#475569", fontSize: 10, fontFamily: "monospace" }}
                      axisLine={false} tickLine={false} />
                    <ZAxis dataKey="congestion_ratio" range={[40, 240]} />
                    <Tooltip {...tt} cursor={{ strokeDasharray: "3 3", stroke: "#1E293B" }} />
