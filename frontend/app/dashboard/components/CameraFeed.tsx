...
export default function CameraFeed() {
  ...
  const load = async () => {
    try {
      const d = await fetchCameraList();
      setCameras(d.cameras); setError(null); setLoading(false);
    } catch { setError("Camera list unavailable"); setLoading(false); }
  };
  ...
  if (!cameras.length) return (
    <Card><p className="text-text-secondary text-sm">Awaiting camera data...</p></Card>
  );

  return (
    <Card className="!p-4">
      <div className="flex items-center justify-between mb-3">
        <p className="text-[10px] text-text-muted font-mono uppercase tracking-widest">Camera Nodes</p>
        <span className="text-[10px] text-text-muted font-mono">{cameras.length} total</span>
      </div>
          ...
          <div key={cam.camera_id}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-text-muted font-mono">
                👤{cam.person_count} · 🚗{cam.vehicle_count} · {Math.round(cam.crowd_density * 100)}%
              </p>
            </div>
